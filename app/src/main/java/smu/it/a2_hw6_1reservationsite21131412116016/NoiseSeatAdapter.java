...
public class NoiseSeatAdapter extends BaseAdapter {
    Context context;
    // 좌석 이미지 배열
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                android.app.AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);
                dialogBuilder.setTitle("--- 소음존 좌석 예약 ---");
                dialogBuilder.setMessage(seatNUM[selectedPosition]+"번 좌석을 예약하시겠습니까?");
                dialogBuilder.setNegativeButton("취소", null);
                dialogBuilder.setPositiveButton("확인",
                        new DialogInterface.OnClickListener() { // 확인 버튼 누르면 이벤트 발생
                            public void onClick (DialogInterface dialog, int which){
                                String seat_num, noise_zone;
                                // 선택한 좌석의 번호를 문자로 형변환
                                seat_num = seatNUM[selectedPosition].toString();
                                noise_zone = "소음존";

                                // 인텐트로 데이터 전달
                                Intent intent = new Intent(context, ReservationResult.class);
                                Bundle extras = new Bundle();
                                extras.putString("zone name", noise_zone);
